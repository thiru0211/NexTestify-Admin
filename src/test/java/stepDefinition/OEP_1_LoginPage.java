...
public class OEP_1_LoginPage {
	...
	public void to_check_login_is_navigating_to_oep_url_is(String url) {
		System.setProperty("webdriver.chrome.driver", "C:\\Users\\thirumaran\\eclipse-workspace\\NexTestify_Admin\\Driver\\chromedriver.exe");
...
//		option.addArguments("--headless=new");
	...
	public void click_the_signin_button_to_check_login() throws InterruptedException {
		Thread.sleep(2000);
	...
	public void click_user_logo_button_in_top_right_corner() throws InterruptedException {
		Thread.sleep(15000);
		wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//img[@class='profile mb-2']")));
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//img[@class='profile mb-2']")));
	...
	public void check_login_page_is_displayed_or_not_using_assert() {
		...
		String expectedUrl = "https://hedgeonline.in/OEPADMIN/Login";
	...
	public void check_alert_message_is_displayed_or_not() throws InterruptedException {
		...
		System.out.println("⚠️ Alert message dispalyed like : " + actualText);
	...
	public void check_alert_message_is_displayed_or_not_in_login_Page() throws InterruptedException {
		try {
		    ...
		    boolean isDisplayed = ele1.isDisplayed();
		    String actualText = ele1.getText();
		    System.out.println("⚠️ Alert message displayed like : " + actualText);
		    Assert.assertTrue("Alert message is not displayed", isDisplayed); 
		} catch (TimeoutException e) {
		    System.out.println("⚠️ Alert message is not displayed and page navigates to next page"); // Print the message here
		    Assert.fail("Alert message is not found and page navigates to next page"); 
		}
	}

	@Then("Click admin button to unlock the account")
	public void click_admin_button_to_unlock_the_account() throws InterruptedException {
		Thread.sleep(5000);
		wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("(//a[@role='button'])[2]")));
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("(//a[@role='button'])[2]")));
		ele1 = driver.findElement(By.xpath("(//a[@role='button'])[2]"));
		Actions action = new Actions(driver);
		action.moveToElement(ele1).build().perform();
		ele1.click();
		wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//ul[@class='collapse show']//a[1]")));
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//ul[@class='collapse show']//a[1]")));
	...
	public void click_unlock_button_to_unlock_the_account() throws InterruptedException {
		wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("(//button[contains(@class,'btn btn-sm')])[3]")));
...
//		js.executeScript("arguments[0].scrollIntoView(true);", ele1);
	...
	public void check_alert_toast_message_is_displayed_in_email_or_not() {
		wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		wait.until(ExpectedConditions
				.presenceOfElementLocated(By.xpath("//div[@class='d-block invalid-tooltip']")));
	...
	public void without_enter_email_id_click_forget_password_button() {
		wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		wait.until(ExpectedConditions
				.presenceOfElementLocated(By.xpath("//span[@class='text-underline text-bold']")));
		ele1 = driver.findElement(By.xpath("//span[@class='text-underline text-bold']"));
		ele1.click();
	}
	...
	public void click_ok_button() throws InterruptedException {
		...
		wait = new WebDriverWait(driver, Duration.ofSeconds(60));
		wait.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//button[contains(@class,'btn btn-sm')]")));
