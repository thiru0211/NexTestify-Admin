Feature: STUDY MATERIAL
  ...
  Background: Study Material
    Given To Check Study Material is navigating to OEP URL is "https://hedgeonline.in/OEPADMIN/Login"
    When To Check Study Material Enter username and password are "thirumaran@sankarasoftware.com" and "Thirumaran@123"
    And click the Signin button To Check Study Material
    Then Click Study Material button

  @TC_01
  Scenario: Study Material:Back Check
    Given Click "Test Subject" subject name
  ...
  Scenario: Add Material:Valid Check
    Given Click "Test Subject" subject name
    When Click add material button
    And Upload valid files in reference tab
  ...
  Scenario: Edit Subject:Close button Check
    Given Click "Test Subject" subject name
    When Click add material button
    And Select "Test Subject" subject from the dropdown in the subject tab
 ...
 Then Close Study Material button
  ...
  @TC_16b
  Scenario: Add Subject:Add button Check
    Given Click "Artificial Intelligence" subject name
    When Click add material button
    And Click add button in the subject tab
    Then Enter "Test Subject" subject name in add subject
    Then Click save button in add subject
    Then Check success message is displayed or not in create material
    Then Close Study Material button
  ...
  Scenario: Add Material:Valid Check
    Given Click "Test Subject" subject name
    When Click add material button
    And Select "Sample Test" subject from the dropdown in the subject tab
...
#Below scripts are commented due to this tasks are not currently working in testing server
  #@TC_27
